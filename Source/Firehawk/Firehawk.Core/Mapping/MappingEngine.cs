...
using Fhwk.Core.Utils.Extensions;
...
namespace Fhwk.Core.Mapping
{
    ...
    internal class MappingEngine
    {
        ...
        {
            var dialect = NHibernate.Dialect.Dialect.GetDialect(nhConfig.Properties);
            if ((dialect != null) && (dialect.GetType().InheritsFrom(typeof(NHibernate.Dialect.MsSql2000Dialect))))
            {
                this.nhConfig.SetNamingStrategy(new FirehawkNamingStrategy());
            }
        }
        ...
        {
            return !this.rootConfig.EntityDefinitionsConfig.RootEntityTypes.Contains(type) && this.rootConfig.EntityDefinitionsConfig.RootEntityTypes.Any(t => t.IsAssignableFrom(type));
        }
        ...
        {
            return this.rootConfig.EntityDefinitionsConfig.RootEntityTypes.Contains(type.BaseType) && !this.rootConfig.EntityDefinitionsConfig.RootEntityTypes.Contains(type);
        }
